<?php
/**
 * File: class-wpglobus-plus-slug.php
 *
 * @since 1.0.0
 */
	...
	class WPGlobusPlus_Slug {
		...
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				//add_filter( 'wpglobus_wpseo_breadcrumb_output', array( $this, 'filter__wpseo_breadcrumb_output' ), 5, 3 );
			}
		}
		...
		public function filter__block_editor_localize_data( $data ) {
			...
			$is_home_page = false;
			if ( (int) $post->ID === (int) $page_on_front ) {
				$is_home_page = true;
			}
			...
			if ( $is_home_page ) {
				...
				$data['switcherItems']['slug'] = array(
					'containerClassName' => 'flex-row',
					'elements' => array(
						array(
							'tagName'  => 'InputSlug',
							'callBack' => 'WPGlobusPlusSlug',
							'props' => array(
								'style'		=> array('marginLeft'=>'26px','width'=>'210px'),
								'className' => '',
								'value' 	=> '',
								'placeholder' => 'URL Slug for {{LanguageName}}',
							)
						),	
					)
				);
			...
			$data['switcherItems']['external-url'] = array(
				'containerClassName' => 'flex-row',
				'elements' => array(
					array(
						'tagName'  => 'ExternalButton',
						'callBack' => 'WPGlobusPlusSlug',
						'props' => array(
							'style'		=> array('marginLeft'=>'26px'),
						...
						'children' => '{{externalUrl}}'
					),	
				)
			);	
			
			$data['switcherItems']['divider'] = array(
				'containerClassName' => '',
				'elements' => array(
					array(				
						'tagName' => 'hr',
		...
		public function check_hierarchical_post_url() {
			...
			/**
			 * Prevent redirect page with alert `Having problems loading Elementor? Please enable Safe Mode to troubleshoot.` for extra language with Elementor.
			 * @since 1.4.2
			 */			
			if ( defined( 'ELEMENTOR_VERSION' ) && ! empty( $wp_query->queried_object->post_type ) && 'page' == $wp_query->queried_object->post_type ) {
				/**
				 * e.g testing string $_SERVER[REQUEST_URI] => /ru/test-page-for-elementor/?elementor-preview=777&ver=1601390795
				 */
				if ( false !== strpos( $_SERVER['REQUEST_URI'], 'elementor-preview=' ) && false !== strpos( $_SERVER['REQUEST_URI'], '&ver=' ) ) {
					return;
				}
			}
		...
		public function filter__wpglobus_wpseo_localize_url( $url, $language, $tag ) {
			...
			if ( ! in_array( $tag, array( 'singular' ) ) ) {
				return $url;
			}
			return $this->filter__wpglobus_pre_localize_current_url( $url, $language );
		}
		...
		public function filter__wpglobus_pre_localize_current_url( $url = '', $language = '' ) {
			...
			if ( $wp_query->is_singular && $wp_query->is_main_query() ) {
				...
				if ( ! empty( $_cache[ $language ] ) ) {
					return $_cache[ $language ];
				}
				// error_log( print_r( '	$wp_query->post->ID 	LOG :: '.$wp_query->post->ID . ' for:: ' . $language, true ) );
		...
		public function on_admin_scripts() {
			...
			if ( WPGlobus_WP::is_pagenow( array( 'post.php', 'post-new.php' ) ) ) :
				 ...
				 * @since 1.5.1
				 */
				$postL10n_obsolete = 'true';
				if ( version_compare( $GLOBALS['wp_version'], '5.5', '<' ) ) {
					$postL10n_obsolete = 'false';
				}
				...
				if ( isset( WPGlobus::Config()->builder ) && WPGlobus::Config()->builder->is_running() ) {
					...
					/**
					 * Get permalink templates.
					 *
					 * @since 1.10.0
					 */
					$permalink_templates = $this->get_permalink_templates($post);
					...
					wp_localize_script(
						...
						array(
							...
							'postSlugs'	   => $this->get_post_slugs($post->ID),
							'permalinkTemplates' => $permalink_templates,
						)
					);
					...
					wp_localize_script(
						...
						'WPGlobusSlug',
						array(
							'postL10n_obsolete' => $postL10n_obsolete,
							'i18n' => $i18n,
							'data' => array(
								'version'      => WPGLOBUS_PLUS_VERSION,
								'process_ajax' => __CLASS__ . '_process_ajax',
		 ...
		 * 
		 * @since 1.0.0
		 * @since 1.10.0 Added `updateSlug`, `getSlug` actions.
		 * 
		 */
		public function on_process_ajax() {
			...
			switch ( $order['action'] ) :
				case 'updateSlug':
					...
					$ajax_return['slug'] 	= $order['value'];
					if ( update_post_meta( $order['post_id'], $this->meta_key . $order['language'], $order['value'] ) ) {
				...
				case 'getSlug':
					...
					$ajax_return['status']  = 'success';
					$ajax_return['payload'] = $order;
					$ajax_return['slug'] = $this->get_post_slug(
						$order['post_id'],
						$order['language']
					);			
		 ...
		 * @since 1.10.0
		 */
		public function get_permalink_templates( $post ) {
			
			if ( ! WPGlobus_WP::is_pagenow('post.php') ) {
				return false;
			}

			$permalink_templates = false;
			...
			if ( $post->post_type === 'page' ) {
				...
				if ( 0 === $parent_page_id ) {
					$permalink_templates = $this->generate_permalink_templates($post);
				} else {
					$permalink_templates = $this->generate_permalink_templates($post, $parent_page_id);
				}
			} else {
				
				/**
				 * Post type: post or CPTs.
				 */
				$permalink_templates = $this->generate_permalink_templates($post);
			}

			return $permalink_templates;	
		}

		/**
		 * Generate permalink templates to send in JS.
		 *
		 * @since 1.10.0
		 */		
		protected function generate_permalink_templates( $post = false, $parent_page_id = false ) {
			...
			if ( 'page' === $post->post_type ) {
				...
				$parent_page_slugs = $this->get_post_slugs($parent_page_id);
				
				if ( $parent_page_slugs ) {
					
					$default_page_slug = false;
					
					foreach( WPGlobus::Config()->enabled_languages as $language ) {
						
						$permalink_templates[$language] = WPGlobus_Utils::localize_url($permalink, $language);
						
						if ( $language === WPGlobus::Config()->default_language ) {
							$permalink_templates[$language] = $permalink;
							$default_page_slug = $this->get_post_slug($post->ID, $language);
						} else {
							
							$permalink_templates[$language] = str_replace( 
								'/'.$parent_page_slugs[WPGlobus::Config()->default_language].'/',
								'/'.$parent_page_slugs[$language].'/',
								$permalink_templates[$language] 
							);
							
						}
						
						if ( $default_page_slug ) {
							$permalink_templates[$language] = str_replace( 
								'/'.$default_page_slug.'/',
								'/'.'%pagename%'.'/',
								$permalink_templates[$language] 
				...
				if ( $post->post_type === 'post' ) {
					 ...
					 * /%postname%/
					 */
					$ml_terms = $this->get_ml_terms();	
				} else {
					
					/**
					 * CPT.
					 */
					$ml_terms = $this->get_ml_terms('post_type', $post->post_type);
				}

				$permalink = get_permalink($post->ID, true);
				...
				foreach( WPGlobus::Config()->enabled_languages as $language ) {
					...
					if ( $language === WPGlobus::Config()->default_language	) {
						...
						if ( $ml_terms ) :
							...
							foreach( $ml_terms as $ml_term ) {
								
								if ( ! empty( $ml_term['mlSlug'][$language] ) ) {
									
									if ( -1 !== strpos( $permalink_templates[$language], '/'.$ml_term['slug'].'/' ) ) {
										
										$permalink_templates[$language] = str_replace( 
											'/'.$ml_term['slug'].'/',
											'/'.$ml_term['mlSlug'][$language].'/',
											$permalink_templates[$language] 
										);
									}
								}
		 ...
		 * @since 1.10.0
		 */
		public function get_ml_terms( $what = 'taxonomy', $name = 'category' ) {

			$ml_terms = false;

			if ( class_exists('WPGlobusPlus_Taxonomies') ) {
				$taxonomies = new WPGlobusPlus_Taxonomies();
				
				if ( 'taxonomy' === $what ) {
					$ml_terms = $taxonomies->get_ml_terms($name);
				} else if ( 'post_type' === $what ) {
					$ml_terms = $taxonomies->get_ml_post_type_slug($name);
				}					
		...
		/**
		 * Get post slug. 
		 * The `post_name` field will be returned for the default language.
		 *
		 * @scope both.
		 *
		 * @since 1.10.0
		 */
		public function get_post_slug( $post_id, $language ) {
		 ...
		 * @since 1.10.0
		 */
		public function get_post_slugs( $post_id ) {
		...
		/**
		 * Get meta key of Slug module.
		 *
		 * @since 1.10.0
		 */
		public function get_meta_key($language = '') {
			if ( '' === $language ) {
				return false;
			}
			return $this->meta_key . $language;
		}
